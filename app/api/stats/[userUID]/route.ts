...
import StatsDB from "../../../utils/Stats";
...
export async function GET(request: Request, { params }: { params: { userUID: string } }) {
    try {
        ...
        const statsData = await StatsDB.find(
...
export async function PUT(nextReq: NextRequest, { params }: { params: { userUID: string } }) {
    try {
        ...
        const result = await StatsDB.updateOne({ userID: params.userUID }, { $set: { totalGames: totalGames, totalWins: totalWins, totalLosses: totalLosses } });
